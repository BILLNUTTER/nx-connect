...
### Render.com
Uses `render.yaml` at the project root. Build: `npm install && npm run build`. Start: `npm start` (runs `dist/index.cjs`).

### Vercel
Uses `vercel.json` at the project root.
- **Build**: `npm run build` (detects `VERCEL=1` env var automatically and skips the esbuild server bundle — Vercel compiles `api/index.ts` itself)
- **Static output**: `dist/public` (Vite build output, served via Vercel CDN)
- **API function**: `api/index.ts` — wraps the full Express app as a serverless handler; routes `/api/*` to this function
